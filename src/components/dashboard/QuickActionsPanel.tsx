...
const QuickActionButton = ({ iconSrc, label, onClick, className }: QuickActionButtonProps) => {
  return (
    <button
      className={cn(
        "flex flex-col items-center justify-center h-24 bg-[#f4f4f5] rounded-lg hover:bg-[#e8e8e9] transition-all duration-200 cursor-pointer group",
        className
      )}
      onClick={onClick}
    >
...
export const QuickActionsPanel = ({ className }: QuickActionsPanelProps) => {
  ...
  return (
    <div className={cn("w-full", className)}>
      <h2 className="text-xl font-semibold text-text-primary mb-4 font-sans">Actions Rapides</h2>
