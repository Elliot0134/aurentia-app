...
const DiscoverySource = ({ selectedSource, onSelect }: DiscoverySourceProps) => {
  const sources = [
    ...
    { id: 'instagram' as DiscoverySourceType, label: 'Instagram', icon: Instagram },
    { id: 'youtube' as DiscoverySourceType, label: 'YouTube', icon: Youtube },
    { id: 'tiktok' as DiscoverySourceType, label: 'TikTok', icon: Music },
    { id: 'friends_school' as DiscoverySourceType, label: 'Amis', icon: GraduationCap },
        ...
        {sources.map((source, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <motion.div
                  className={`p-3 rounded-xl ${isSelected ? 'bg-[#FF6B35]/10' : 'bg-gray-100'}`}
                  animate={{ scale: isSelected ? [1, 1.1, 1] : 1 }}
                  transition={{ duration: 0.3 }}
                >
                  <Icon className={`w-6 h-6 ${isSelected ? 'text-[#FF6B35]' : 'text-gray-700'}`} />
                </motion.div>
                <span className={`text-sm font-poppins font-semibold ${isSelected ? 'text-[#FF6B35]' : 'text-[#333333]'}`}>
                  {source.label}
                </span>
              </motion.div>
            </motion.button>
